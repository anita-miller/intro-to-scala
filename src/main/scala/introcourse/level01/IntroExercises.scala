...
object IntroExercises {
  ...
  val fn2: Int => (Int => Int) = addCurried
...
//  List(1,2).map(x => add(5, x))
//  List(1,2).map(addCurried(5))
  ...
  val a: String = foo[String]("hello")
  val b: Int = foo(1)

  /**
    * How about this one?
    */
  def bar(a: Int): Int = 100
